...
inning7orlater_within1_less2strikes <- inning7orlater_within1 %>% 
  filter(strikes != 2) %>% 
  group_by(cluster, base_sit, outs_when_up) %>% 
  reframe(delta_bat_win_exp = mean(delta_bat_win_exp)) %>% 
  distinct() %>% 
...
inning7orlater_within1_2strikes <- inning7orlater_within2 %>% 
  filter(strikes == 2) %>% 
  group_by(cluster, base_sit, outs_when_up) %>% 
  reframe(delta_bat_win_exp = mean(delta_bat_win_exp)) %>% 
  distinct() %>% 
...
delta_win_shap_long <- shap.prep(xgb_model = delta_win_exp_xg_model, X_train = as.matrix(delta_win_exp_model_df_x))
shap.plot.summary(delta_win_shap_long) +
  labs(title = 'Delta Win Expectancy Model SHAP Plot')
shap.plot.dependence(delta_win_shap_long, x = 'bat_score_diff')
shap.plot.dependence(delta_win_shap_long, x = 'cluster_3', smooth = FALSE)
